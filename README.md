...
```bash
./scripts/join-agent.sh <MAC_MINI_IP> <TOKEN>
# Optional: if nodes are on different networks, add gpu-node-1's external IP
./scripts/join-agent.sh <MAC_MINI_IP> <TOKEN> <GPU_NODE_IP>
```
...
- **kubeconfig server unreachable**: The default kubeconfig may use localhost; from Mac Mini, `127.0.0.1:6443` should work via port mapping
